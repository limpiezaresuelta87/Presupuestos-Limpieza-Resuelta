...
(function () {
  ...
  function actualizarListaDesdeArchivo(file) {
    return leerArchivoExcel(file).then(function (workbook) {
      ...
      // Además, intentamos leer la hoja de costos del MISMO archivo para
      // sumar un snapshot mensual (para los informes de ganancia). Esto es
      // "best effort": si no se encuentra o no pasa la validación de
      // sanidad, no pasa nada -- la lista de precios recién guardada arriba
      // no se ve afectada de ninguna forma.
      let snapshotCostos = null;
      try {
        const resultadoCostos = parsearHojaCostos(workbook);
        if (resultadoCostos && resultadoCostos.items.length > 0) {
          snapshotCostos = {
            mes: new Date().toISOString().slice(0, 7), // "YYYY-MM"
            fecha: new Date().toISOString(),
            origenArchivo: file.name + ' (hoja: ' + resultadoCostos.hojaUsada + ')',
            items: resultadoCostos.items,
          };
          window.Store.agregarSnapshotCostos(snapshotCostos);
        }
      } catch (errCostos) {
        console.warn('No se pudo leer la hoja de costos de este Excel:', errCostos);
      }

      return { lista: lista, filasIgnoradas: resultado.filasIgnoradas, snapshotCostos: snapshotCostos };
    });
  }
